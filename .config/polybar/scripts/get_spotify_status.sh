...
if [ "$status" = "No Player" ]; then
    ...
    polybar-msg -p "$(pgrep -f "polybar --reload main1")" hook spotify-play-pause 2 1>/dev/null 2>&1
    playerctl --player=playerctld metadata --format "{{ artist }}"
else # Can be configured to output differently when player is paused
    polybar-msg -p "$(pgrep -f "polybar --reload main1")" hook spotify-play-pause 1 1>/dev/null 2>&1
